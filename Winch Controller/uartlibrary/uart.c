...
**************************************************************************/
{
	...
	if(LINSIR & (1<<LENTXOK)){
		if ( UART_TxHead != UART_TxTail) {
			...
			/* tx buffer empty, TX complete interrupt */
			LINENIR &= ~(1<<LENTXOK);
...
//set baudrate
LINBRRH = (((F_CPU/baudrate)/16)-1)>>8;
LINBRRL = (((F_CPU/baudrate)/16)-1);
//disable bit resinchronization & set bit sampling to 16
